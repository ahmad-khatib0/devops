# This Packer file defines variables for GCP, the base image used in the
# install, the name of the resulting image, the properties of the virtual machine used
# to build the image, and the steps in provisioning. The machine type used to create
# the image is unrelated to the machine that is instantiated from that image. We use
# powerful machines to create complex images, at the same cost, but the job is done quickly.
